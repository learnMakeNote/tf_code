...
resource "azurerm_linux_virtual_machine" "vm_linux" {
  count = local.is_linux ? var.linux_instance_count : 0
  ...
  computer_name                   = coalesce(var.computer_name, "${var.name}-${count.index}")
...
resource "azurerm_windows_virtual_machine" "vm_windows" {
  count = local.is_windows ? var.windows_instance_count : 0
...
resource "azurerm_network_interface" "vm" {
  count = var.new_network_interface != null ? var.new_network_interface.network_interface_number : 0

  location                      = var.location
  name                          = coalesce(var.new_network_interface.name, "${var.name}-nic-${count.index}")
  resource_group_name           = var.resource_group_name
  ...
  dynamic "ip_configuration" {
    ...
    content {
      name                                               = coalesce(var.new_network_interface.ip_configurations[ip_configuration.value].name, "${var.name}-nic${ip_configuration.value}-${count.index}")
      ...
      public_ip_address_id                               = var.new_network_interface.ip_configurations[ip_configuration.value].public_ip_address_id[count.index]
...
# get all network_interface_id
#locals {
#  network_interface_ids = var.new_network_interface != null ?
#  [for k, v in azurerm_network_interface.vm: v.id] :
